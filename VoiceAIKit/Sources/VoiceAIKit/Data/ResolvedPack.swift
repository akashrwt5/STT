...
struct ResolvedPack: Sendable {
    ...
    let lexicon: PackLexicon
    let keywordRules: [PackKeywords.Rule]
    ...
    struct ClassifierArtifacts: Sendable {
        ...
        let confidenceGapThreshold: Double
...
extension ResolvedPack {
    ...
    /// `never` and say so.
    var uncertainConfirmBand: (floor: Double, ceiling: Double)? {
    ...
    func stageEnabled(_ stage: Stage) -> Bool { cascade.isEnabled(stage.rawValue) }
    ...
    var intentIDs: Set<String> { Set(intents.keys) }
    ...
    ///
    /// `Default Fallback Intent` is matched too, and deliberately: it is the
    /// label every shipping pack still trains (it is one of the en pack's 57),
    /// and it is the name the host's own intent table already speaks from its
    /// Dialogflow days. Recognising only `*.oos.fallback` made this nil for every
    /// current pack, which turned the classifier's substituted out-of-scope label
    /// into an empty string — a value that then matched nothing downstream.
    var outOfScopeIntent: String? {
        classifier.labels.first {
            $0.hasSuffix(".oos.fallback")
                || $0 == "sys.oos.fallback"
                || $0 == NLUSchema.defaultFallbackIntent
        }
    }
    ...
    struct ResolvedSlot: Sendable {
        let spec: IntentWorkflow.SlotSpec
        let prompt: String
    }
    ...
    func slots(for intent: String) -> [ResolvedSlot] {

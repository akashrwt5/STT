...
struct PackLexicon: Decodable, Sendable {
    ...
    /// this table entirely.
    ///
    /// CONSUMED by `PackTextNormalizer`, which `PackTFIDFVectorizer` applies
    /// before tokenising. It was decoded and read by nothing for three
    /// releases, so the device scored a surface form the head was never fitted
    /// on — see that file for what the divergence measured.
    let contractions: [String: String]
    /// "memories" → "memory". GENERATED by `nlu_training.fit_lemmas` from the
    /// corpus and shipped as data, exactly like `vocab` and `idf`.
    ///
    /// LOAD-BEARING, not cosmetic: the trainer folds the text BEFORE fitting,
    /// so the plural is not a vocabulary column at all. A runtime that skips
    /// the fold hands the head an unknown token and silently loses the
    /// feature. Empty on a pack built before pack-en-v1.0.55, which makes the
    /// stage a no-op rather than a half-applied transform.

...
// for the `VoiceAIKit` target, so SwiftPM does not even synthesise a
...
final class PackageResourceInvariantTests: XCTestCase {
    ...
    /// `Sources/VoiceAIKit/` contains Swift sources and nothing else.
    func testLibraryTargetShipsNoDataFiles() throws {
        ...
        XCTAssertTrue(
            unexpected.isEmpty,
            """
            The VoiceAIKit target must ship no data — every byte it classifies \
            with comes from a host-supplied, verified pack. Unexpected file(s) under \
            Sources/VoiceAIKit/: \(unexpected.sorted()). A model, schema, lexicon \
    ...
    private static var librarySourcesDirectory: URL {
        URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()   // VoiceAIKitTests
            .deletingLastPathComponent()   // Tests
            .deletingLastPathComponent()   // VoiceAIKit (package root)
            .appendingPathComponent("Sources")
            .appendingPathComponent("VoiceAIKit")
    }
}

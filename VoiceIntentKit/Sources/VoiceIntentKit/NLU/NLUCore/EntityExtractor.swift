...
public final class EntityExtractor: @unchecked Sendable {
    ...
    public init(entitiesURL: URL? = nil,
                lexicon: NLULexicon? = nil) {
        // Default to the package's own bundled nlu_entities.json when the caller
        // doesn't specify one. `Bundle.module` is internal, so it cannot appear in
        // a public default argument — resolve it here instead.
        let resolvedURL = entitiesURL
            ?? Bundle.module.url(forResource: "nlu_entities", withExtension: "json")
        guard
            let url = resolvedURL,
    ...
    private func extractDateTimeLexicon(_ text: String, now: Date, lex: LexTables) -> DateTimeMatch? {
        ...
        //
        // Runs BEFORE D1 (digit + spaced clock-hour marker) because for FR
        // "dix heures et demie" — normalised to "10 heures et demie" — D1's
        // greedy match on "10 heures" would consume the digits and leave D3
        // with no adjacent hour to bind to. Trying idioms first lets
        // `lexAdjacentHour` find "10" while skipping the intervening hour
        // marker ("heures"). Utterances without an idiom just fall through
        // to D1 unchanged, so pure "18 heures" still parses to 18:00.
        if hour == nil {

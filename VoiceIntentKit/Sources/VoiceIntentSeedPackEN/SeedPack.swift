...
//
// WHY THIS IS A SEPARATE TARGET FROM `VoiceIntentKit`
//
// `VoiceIntentKit` ships zero data — that is the point of the whole refactor,
...
// `VoiceIntentKit` is a different module with a different bundle and literally
// cannot see this pack.
//
// It also makes the seed OPTIONAL. Xcode asks which of a package's libraries to
// link; an app that downloads every language ticks only `VoiceIntentKit` and

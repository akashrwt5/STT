...
> The `VoiceIntentKit` SDK is completely agnostic to networking and background task scheduling. It is strictly the responsibility of the Host Application to communicate with the backend API, download the OTA `.nlu` zip payload, and schedule background tasks using `BGTaskScheduler`. 
> 
...
```
*(Note: Cryptographic integrity is verified via Ed25519 signatures inside the `.nlu` bundle itself by `VoiceIntentKit`, so `sha256_hash` may be omitted).*

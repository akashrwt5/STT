# VoiceIntentKit — public API reduction plan
...
| | Count |
|---|---|
| Declarations starting with `public` / `open` in `Sources/VoiceIntentKit` | **745** |
...
`PackIdentity`, `VoiceIntentKit.verifyPack(...)`, `VoiceIntentKit.smokeTest(...)`.
...
**Tests keep working.** Every test file already uses `@testable import VoiceIntentKit`, which sees internal symbols. No test should need editing for the demotion itself — if one does, that is a signal the type belonged to the facade after all. Note it rather than making it public again.

**Temporary escape hatch.** For anything the STT app still touches mid-migration:

```swift
@_spi(VoiceIntentKitInternal) public func …          // in the package
@_spi(VoiceIntentKitInternal) import VoiceIntentKit  // in the host
```
...
jq -r '.symbols[] | select(.accessLevel == "public") | (.pathComponents | join("."))' \
   .build/*/symbolgraph/VoiceIntentKit.symbols.json | sort -u > docs/public-api.txt

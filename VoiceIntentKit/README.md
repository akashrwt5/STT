...
`VoiceIntentKit` target, so SwiftPM does not even synthesise a `Bundle.module` for it — there
...
| Product | What it is |
|---|---|
| `VoiceIntentKit` | The engine. Zero data. Link this always. |
...
open `Package.swift` in Xcode and press Cmd+U, or `xcodebuild test -scheme VoiceIntentKit

...
| ~~C3~~ | `.fallback` → `.unresolved`; providers **MUST NOT** attempt fallback | **RESOLVED (VIK-031).** The URL is gone rather than made optional. It was built from unsigned pack data with the user's verbatim transcript in its query string — the only path in the kit that reached the network. `.notUnderstood(intent:confidence:stages:)` carries the pack's own out-of-scope label instead, which the host's intent table already dispatches. The SPEC and HLD were re-baselined on 21 Aug; nothing is left to discard. |
...
| ~~C5~~ | §"exactly one terminal event per turn" | **RESOLVED (21 Aug) — and my original diagnosis here was wrong.** I wrote this as the kit deviating from the SPEC. It does not. The SPEC contradicted *itself*: §2.3 (`.abandoned` — "the outcome for the new utterance arrives as the NEXT `.dialogue` event"), §6.2's mapping table (`.interrupted` → `.abandoned`, "then map `inner` as the next event") and Test Strategy IT-11 all describe two events; only §3.2.6 and the §3.1 state diagram said one. Three against two, and the kit matched the three — which were also the ones written against its actual behaviour. Fixed in the SPEC, not the kit: §3.2.6 now distinguishes turn-ending from non-terminal events, the state diagram sends `.abandoned` back to `Processing` rather than `Ready`, and CF-14 asserts the sequence. `.abandoned` **MUST NOT** be suppressed — the host has UI bound to the dialogue that died. Zero code changed. |
...
**Phase 2 — Engage adapter.** `VoiceUnderstandingProvider` over `.appProvided` + `speaksPrompts: false`; host runs CMS → GenAI → Wolfram on `.unresolved`; PTT stays app-owned with `resetDialogue()` before dispatch. SPEC re-baseline done for C3 and C5 (21 Aug); **C4 still open** — the SPEC says `audioSource: .injected(...)`, the kit ships `.appProvided(sampleRate:)`.

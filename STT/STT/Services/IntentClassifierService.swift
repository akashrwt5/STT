...
// Stage 2 — TF-IDF + LogReg (CoreML intent model)    sync, ~2 ms
...
public final class IntentClassifierService: @unchecked Sendable {
    ...
    private init() {
        ...
        if let modelURL = Bundle.main.url(forResource: "IntentClassifier",
    ...
    private func coreMLClassify(_ text: String, model: MLModel) -> (String, Double)? {
        // The ONNX→CoreML pipeline bakes the TF-IDF vectoriser in as a string input.
        // coremltools names the input "input" and produces "classProbability" dict output.
        guard
            let input    = try? MLDictionaryFeatureProvider(dictionary: [
                "input": MLFeatureValue(string: text)
            ]),
        ...
        guard let best = probDict.max(by: { $0.value < $1.value }) else { return nil }
